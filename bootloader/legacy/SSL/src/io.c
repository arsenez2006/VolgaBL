    ...
    while (*format) {
        if (*format != '%') {
            ...
            case 'd': {
                arg.intmax_type = 0U;
                if (flags & FLAG_CHAR)
                    arg.char_type = va_arg(va, char);
                else if (flags & FLAG_SHORT)
                    arg.short_type = va_arg(va, short);
                #ifdef _VLGBL_LONG
            ...
            case 'X': {
                arg.uintmax_type = 0U;
                if (flags & FLAG_CHAR)
                    arg.uchar_type = va_arg(va, unsigned char);
                else if (flags & FLAG_SHORT)
                    arg.ushort_type = va_arg(va, unsigned short);
                #ifdef _VLGBL_LONG
                else if (flags & FLAG_LONG)
