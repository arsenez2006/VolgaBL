...
static const byte_t null_partition_type[] = { 0x00, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x00 };
...
_check_A20(void) {
    ...
    const word_t failure_offset =
      (((dword_t)0xFFFF << 4) + test_offset) & 0xFFFF;
...
get_partition_array(const GPT_header* gpt_hdr) {
    ...
    while (true) {
        ...
        while (li < gpt_hdr->entries_count &&
            ...
            ++li;
            lp = (GPT_partition_entry*)(((byte_t*)lp) +
        ...
        while (ri < gpt_hdr->entries_count) {
            ...
            ++ri;
            rp = (GPT_partition_entry*)(((byte_t*)rp) +
                                        partition_array->entry_size);
        }
    ...
    if ((partition_array->array = realloc(partition_array->array,
                                          partition_array->entry_size *
                                            partition_array->count)) == NULL) {
        free(partition_array->array);
        free(partition_array);
        return NULL;
    }
